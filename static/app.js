...
const TIER = {
  A: { label: "A", color: "#3cc972", desc: "缓存/日志/残包/空目录，可直接清" },
  B: { label: "B", color: "#e5a443", desc: "不确定是否安全，请人工核对" },
  C: { label: "C", color: "#e668a7", desc: "完整安装程序，请走卸载器" },
  D: { label: "D", color: "#f25c5c", desc: "个人数据/系统关键，禁止删除" },
};
...
function switchTab(id) {
  ...
  localStorage.setItem("wdc-tab", id);
}
...
async function loadDisk() {
  ...
  (d.sys_files || []).forEach(f => {
    html += '<div class="sys-row">' +
      ...
      '<span class="value">' + (f.size != null ? human(f.size) : "不存在") + '</span>' +
      '</div>';
  });
  if (d.upgrades && d.upgrades.length) {
    html += '<div class="sys-row warn">' +
      '<span class="label">升级残留</span>' +
      '<span class="value">' + esc(d.upgrades.join(", ")) + '</span>' +
      '</div>';
  }
  const h = d.hibernate;
  html += '<div class="sys-row">' +
    '<span class="label">休眠文件</span>' +
    '<span class="value">' + (h == null ? "读不到" : h ? "开启" : "关闭") + '</span>' +
    '</div>';
  sys.innerHTML = html;
...
async function doScan() {
  ...
  if (!path) { el("scanStatus").textContent = "请先填写要扫描的目录"; return; }
  el("btnScan").disabled = true;
  el("scanStatus").textContent = "扫描中…（限时 " + el("scanBudget").value + " 秒）";
  selected.clear();
...
function renderResults(res) {
  ...
  sum.innerHTML = TIER_ORDER.map(t =>
    ...
    TIER[t].label + ' · ' + counts[t] + ' 项 / ' + human(sizes[t]) + '</span>'
  ).join("");

  const wrap = el("resultTable");
  wrap.innerHTML = "";
  if (!res.items.length) {
    wrap.innerHTML = '<div class="empty-state"><span class="big">📂</span>没有匹配的可清项</div>';
    updateSel();
    return;
  }
  ...
  TIER_ORDER.forEach(t => {
    ...
    grp.innerHTML = '<h3><span class="tag" style="background:' + TIER[t].color +
      ...
      '<div class="table-wrap"><table><thead><tr><th style="width:34px"></th><th>名称</th><th>大小</th>' +
      '<th>文件数</th><th>说明</th></tr></thead><tbody></tbody></table></div>';
    const tb = grp.querySelector("tbody");
    items.forEach(it => {
      const tr = document.createElement("tr");
      if (t === "D") tr.className = "disabled";
      const cb = t === "D" ? "" : '<input type="checkbox" data-path="' + esc(it.path) + '">';
...
function renderReg(items) {
  ...
  tbl.innerHTML = '<table><thead><tr><th style="width:34px"></th><th>名称</th><th>发布者</th>'
    + '<th>大小</th><th>路径</th><th>说明</th></tr></thead><tbody></tbody></table>';
...
function renderPrograms(items) {
  ...
  wrap.innerHTML = "";
  if (!items.length) {
    wrap.innerHTML = '<div class="empty-state"><span class="big">🗑️</span>请先列出已安装程序，或筛选无结果</div>';
    return;
  }
  const tbl = document.createElement("div");
  tbl.className = "table-wrap";
  tbl.innerHTML = '<table><thead><tr><th>名称</th><th>发布者</th><th>大小</th>'
    + '<th style="width:110px">操作</th></tr></thead><tbody></tbody></table>';
  const tb = tbl.querySelector("tbody");
  items.forEach(it => {
    const tr = document.createElement("tr");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const saved = localStorage.getItem("wdc-tab");
  if (saved) switchTab(saved);

  el("btnRefreshDisk").addEventListener("click", loadDisk);
